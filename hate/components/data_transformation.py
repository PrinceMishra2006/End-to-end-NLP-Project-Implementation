...
class DataTransformation:
    ...
    def imbalance_data_cleaning(self):
        try:
            logging.info("Entered init the imbalance_data_cleaning function")
            ...
            logging.info("Cleaning the imbalance data")
    ...
    def concat_dataframe(self):
        try:
            logging.info("Entered into the concat_data function")
            # Let's concatinate both the data into a single data frame.
            frame = [self.raw_data_cleaning(), self.imbalance_data_cleaning()]
            df = pd.concat(frame)
            print(df.head())
            
            logging.info(f"returning the concatinated data {df}")
            return df
